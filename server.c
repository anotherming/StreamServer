...
void* _server_worker (void *args) {
	...
	while (server->shutdown == false) {
		...
		if (_check_movie (&request, server) == false) {
			...
			response.data = malloc ((strlen (NSG_NOT_EXIST) + 1) * sizeof (char));
			strcpy (response.data, NSG_NOT_EXIST);

			response.len = strlen (NSG_NOT_EXIST) + 1;
	...
	while (server->shutdown == false) {
		...
		if (server->shutdown == false 
			...
			while (sortlist != NULL) {
				...
				// block and write
				_write_response (entry->response.request.client_sd, (void *)&entry->response);
